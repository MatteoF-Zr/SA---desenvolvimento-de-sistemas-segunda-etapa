...
public class AplicativoService {
    ...
    public AplicativoDTO salvar(AplicativoDTO dto) {
        if (dto.getNom_aplicativo() == null || dto.getNom_aplicativo().trim().isEmpty())
            throw new IllegalArgumentException("nom_aplicativo é obrigatório.");
        if (dto.getTax_plataforma() == null || dto.getTax_plataforma().signum() < 0)
            throw new IllegalArgumentException("tax_plataforma inválida.");
        if (dto.getTax_cancelamento() != null && dto.getTax_cancelamento().signum() < 0)
            throw new IllegalArgumentException("tax_cancelamento inválida.");
        if (dto.getEma_suporte() != null && !dto.getEma_suporte().contains("@"))
            throw new IllegalArgumentException("ema_suporte inválido.");

        Aplicativo a = new Aplicativo();
        a.setNom_aplicativo(dto.getNom_aplicativo());
    ...
    public AplicativoDTO atualizar(Integer id, AplicativoDTO dto) {
        ...
        if (dto.getNom_aplicativo() != null) existente.setNom_aplicativo(dto.getNom_aplicativo());
        if (dto.getTax_plataforma() != null) {
            if (dto.getTax_plataforma().signum() < 0) throw new IllegalArgumentException("tax_plataforma inválida.");
            existente.setTax_plataforma(dto.getTax_plataforma());
        }
        if (dto.getTax_cancelamento() != null) {
            if (dto.getTax_cancelamento().signum() < 0) throw new IllegalArgumentException("tax_cancelamento inválida.");
            existente.setTax_cancelamento(dto.getTax_cancelamento());
        }
        if (dto.getEma_suporte() != null) {
            if (!dto.getEma_suporte().contains("@")) throw new IllegalArgumentException("ema_suporte inválido.");
